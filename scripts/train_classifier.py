...
---
### **`matsushibadenki/snn4/SNN4-052f56efa1c6d72aa3fc9bc2e46d3141b9636836/snn-cli.py`**

`benchmark` という新しいコマンドグループを追加し、その配下に `train` サブコマンドを実装しました。
...
#
# 改善点 (v14):
# - ベンチマーク用の分類モデルを訓練するための `benchmark train` コマンドを追加。
...
準備が整いました。以下の2ステップで、SNNモデルの性能証明を実行してください。

#### **ステップ1：SNNモデルの訓練**

まず、新しく追加した `benchmark train` コマンドを使って、SST-2タスクでSNNモデルを訓練します。訓練が完了すると、最も性能の良かったモデルが `runs/classifiers/sst2/best_model.pth` に自動的に保存されます。
...
次に、`run_benchmark.py` を実行します。今回は `--model_path` オプションで、ステップ1で保存した**学習済みモデル**のパスを指定します。

```bash
python scripts/run_benchmark.py --task sst2 --model_path runs/classifiers/sst2/best_model.pth
